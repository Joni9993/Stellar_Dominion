...
function IntroMockup() {
  const factions: { color: string; pos: [number, number]; rot: number }[] = [
    { color: 'var(--teal)', pos: [50, 12], rot: 180 },
    { color: 'var(--amber)', pos: [82, 28], rot: 225 },
    { color: 'var(--red)', pos: [82, 72], rot: 270 },
    { color: '#7aacff', pos: [50, 88], rot: 0 },
    { color: 'var(--purple)', pos: [18, 72], rot: 90 },
    { color: 'var(--green)', pos: [18, 28], rot: 135 },
  ];

  return (
    <div style={{ position: 'relative', height: 155, background: 'var(--void)', border: '1px solid var(--line)', overflow: 'hidden' }}>
      {/* Dashed approach lines from each faction to center */}
      ...
      {/* Faction ships */}
      {factions.map(({ color, pos, rot }, i) => (
        <div key={i} style={{
          position: 'absolute',
          left: `${pos[0]}%`, top: `${pos[1]}%`,
          transform: `translate(-50%, -50%) rotate(${rot}deg)`,
          color, fontSize: 13, lineHeight: 1,
        }}>▲</div>
      ))}

      {/* Central glowing Artifact */}
      <div style={{
      ...
      {/* Corner labels */}
      <div style={{ position: 'absolute', top: 6, left: 8, fontSize: 8, color: 'var(--faint)', fontFamily: "'Press Start 2P'" }}>
        18 SYSTEMS
      </div>
      <div style={{ position: 'absolute', bottom: 6, right: 8, fontSize: 8, color: 'var(--amber)', fontFamily: "'Press Start 2P'" }}>
        RUMORED ARTIFACT
      </div>
    </div>
  );
    ...
    <div style={{ position: 'relative', height: 155, background: 'var(--void)', border: '1px solid var(--line)', overflow: 'hidden' }}>
      ...
      {sys.map(([x, y], i) => (
        <div key={i} style={{
          ...
          left: `${x}%`, top: `${y}%`,
          transform: 'translate(-50%, -50%)',
          width: i === rumorIdx ? 11 : 7, height: i === rumorIdx ? 11 : 7,
          borderRadius: '50%',
      ...
      <div style={{
        position: 'absolute',
        left: `${sys[7][0]}%`, top: `${sys[7][1]}%`,
        transform: 'translate(-50%, -160%)',
        color: 'var(--teal)', fontSize: 11, lineHeight: 1,
      }}>▲</div>
...
function StationMockup() {
  ...
  return (
    <div style={{ height: 155, background: 'var(--panel)', border: '1px solid var(--line)', overflow: 'hidden' }}>
      <div style={{ padding: '6px 10px', borderBottom: '1px solid var(--line)', fontSize: 9, color: 'var(--bone)', fontFamily: "'Press Start 2P'" }}>
        STATION · KEPLER-7
      </div>
  ...
  return (
    <div style={{ height: 155, display: 'flex', flexDirection: 'column', gap: 4 }}>
      <div style={{
        ...
        padding: '6px 10px', display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap',
      }}>
      ...
      <div style={{
        flex: 1, background: 'var(--void)', border: '1px solid var(--line)',
        display: 'flex', alignItems: 'center', justifyContent: 'center',
      }}>
...
const STEPS: TourStep[] = [
  {
    ...
    mockup: <IntroMockup />,
    annotations: [
      { label: 'The Artifacts', detail: 'Ancient relics of a vanished civilisation — scattered across 18 star systems. Whoever controls them controls the Dominion. Their location is Rumored to all. Their possession belongs to none.' },
      { label: 'By Any Means', detail: 'Six factions. Every deal can become a trap. Every ally becomes a target the moment they claim an Artifact. Trade, combat, intrigue — all are valid paths.' },
      { label: 'Claim Victory', detail: 'Be the first to collect enough Artifacts and the Dominion is yours.' },
    ],
  },
  {
    screen: 'STAR MAP',
    ...
    annotations: [
      { label: 'Systems', detail: '18 star systems connected by jump lanes. Click any system to see its details and plan your route.' },
      { label: 'Rumor Ping', detail: 'The glowing amber node is the active Rumored Artifact location. Only one Rumor exists at a time — everyone converges here.' },
    ],
  },
  {
    screen: 'MAP PANEL',
    ...
    annotations: [
      { label: 'Jump Here', detail: 'Move your ship to the selected system. Costs fuel — adjacent systems cost 2⛽, farther ones cost more.' },
      ...
      { label: 'Attack', detail: 'Challenge a rival in the same system to auto-battle. The winner loots a module, artifact, or credits from the loser.' },
    ],
  },
  {
    screen: 'SHIPYARD',
    ...
    annotations: [
      { label: '3×3 Grid', detail: '9 slots for weapons, defenses, and energy modules. BOW zone boosts weapons; placing an Energy module adjacent to a weapon gives +15% fire rate to both.' },
      { label: 'Artifacts', detail: 'Claimed Artifacts must be slotted into the grid to activate their combat effect. They compete with weapons for grid space.' },
    ],
  },
  {
    screen: 'STATION',
    ...
    annotations: [
      ...
      { label: 'Fuel', detail: 'Refuel your ship here. Each jump burns fuel — running out strands you until you reach a station.' },
      { label: 'Crew', detail: 'Hire crew for persistent bonuses. Trader boosts sell prices, Navigator extends jump range, Gunner improves accuracy.' },
      { label: 'Modules', detail: 'Buy ship modules to add to your inventory, then equip them in the Shipyard. Sell unused modules here for 50% of their buy price.' },
    ],
  },
  {
    screen: 'HUD',
    ...
    mockup: <HudMockup />,
    annotations: [
      { label: 'Credits ◈', detail: 'Your main currency — spend on fuel, modules, trade goods, and crew. You start every game with 500◈.' },
      { label: 'Fuel ⛽', detail: 'Consumed on every jump. Running out leaves you stranded — always check before a long route.' },
      { label: 'Cargo', detail: 'Caps how many units of trade goods you can carry at once. Buy a Cargo Bay module at a station to increase it.' },
    ],
  },
  {
    screen: 'COMBAT',
